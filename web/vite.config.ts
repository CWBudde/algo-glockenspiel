...
/**
 * serveDistArtifacts hands the dev server the two files that only the
 * production build produces.
 *
 * `npm run dev` serves from web/, but glockenspiel.wasm and manifest.json are
 * written to web/dist by scripts/build-wasm.sh, so without this the documented
 * hot-reload workflow cannot instantiate the module however often you build.
