...
It writes the fitted preset to `--output` and turns `--work-dir` into a run directory: `config.json`, `analysis.json`, `reference.wav`, `trace.jsonl`, `checkpoint.json`, `preset.json`, `render.wav`, `result.json` and `log.txt`, the same files a campaign job and a fit started from `glockenspiel serve` leave behind.
