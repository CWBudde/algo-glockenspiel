...
func TestFitRunsAndProducesAPresetAndAudio(t *testing.T) {
	...
	// The loader cuts the reference at its onset, a handful of samples into
	// the synthetic strike, so the length it reports is a hair under the
	// upload's.
	if math.Abs(accepted.ReferenceSeconds-testReferenceLength) > 1e-3 {
	...
	// The loader cut the reference at its onset, a handful of samples in, so
	// the render is the cut's length rather than the upload's.
	wantSamples := int(testReferenceLength * testSampleRate)
	if samples := len(samples); samples > wantSamples || samples < wantSamples-64 {
		t.Fatalf("rendered %d samples, want the reference's %d less the onset cut", samples, wantSamples)
	}
}
