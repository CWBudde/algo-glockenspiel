...
func TestFitSeedsTheModesFromTheReferenceAndReportsIt(t *testing.T) {
	...
	if len(fitted.Parameters.Modes) != 2 || fitted.Version != preset.VersionV2 {
		t.Fatalf("fitted preset has %d modes in version %q, want 2 in %q", len(fitted.Parameters.Modes), fitted.Version, preset.VersionV2)
	}
